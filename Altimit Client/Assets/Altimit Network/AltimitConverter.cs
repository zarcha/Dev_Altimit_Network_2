...
namespace Altimit{
	public class AltimitConverter : MonoBehaviour {
			...
			if (methodName != null || methodName != "") {
				...
				foreach(object paramater in args){
					...
					case "Guid":
						...
						break;
					case "Vector3":
						Vector3 castVector3 = (Vector3)paramater;
						currentArr = convertToByteArray (castVector3);
						break;
					case "Quaternion":
						Quaternion castQuaternion = (Quaternion)paramater;
		...
		public static byte[] convertToByteArray(Vector3 value){
			byte[] buffer = new byte[13];
			int position = 0;
			byte[] val;
			float[] values = { value.x, value.y, value.z };

			buffer [position] = (byte)10;
			position++;

			for (int i = 0; i < 3; i++) {
				val = BitConverter.GetBytes (values[i]);
				if (BitConverter.IsLittleEndian) {
					Array.Reverse (val);
				}
				for (int j = 0; j < val.Length; j++, position++) {
					buffer [position] = val[j];
				}
			}
		...
		public static byte[] convertToByteArray(Quaternion value){
		...
		public static List<object> ReceiveConversion(byte[] array){
			...
			for (int i = 0; i < array.Length; i++) {
				...
				case 9:
					...
					break;
				case 10:
					paramaters.Add (convertToVector3 (AltimitArray.copyOfRange (array, i + 1, i + 13)));
					i += 12;
					break;
				case 11:
					paramaters.Add (convertToQuaternion (AltimitArray.copyOfRange (array, i + 1, i + 17)));
					i += 16;
					break;
				}
			}
		...
		public static Vector3 convertToVector3(byte[] array){
			byte[] bx = AltimitArray.copyOfRange (array, 0, 4);
			byte[] by = AltimitArray.copyOfRange (array, 4, 8);
			byte[] bz = AltimitArray.copyOfRange (array, 8, 12);

			Vector3 val = new Vector3 (convertToFloat(bx), convertToFloat(by), convertToFloat(bz));

			return val;
		}

		public static Quaternion convertToQuaternion(byte[] array){
			byte[] x = AltimitArray.copyOfRange (array, 0, 4);
			byte[] y = AltimitArray.copyOfRange (array, 4, 8);
			byte[] z = AltimitArray.copyOfRange (array, 8, 12);
			byte[] w = AltimitArray.copyOfRange (array, 12, 16);

			Quaternion val = new Quaternion (convertToFloat(x), convertToFloat(y), convertToFloat(z), convertToFloat(w));
