...
namespace Altimit {
	...
	public class AltimitNetwork : MonoBehaviour {
        ...
        private static void CreateTaskObject()
        {
            GameObject temp = new GameObject("Tasker");
            temp.AddComponent<TaskManager>();
            tasker = TaskContainer.GetInstance();
        }
		...
		public static void Instantiate(string prefabName, Vector3 position, Quaternion rotation){
			int subId = GameObject.FindObjectsOfType<AltimitView> ().Length;
